...
if manager.latest_checkpoint:
    ...
    model([np.random.rand(1, 640, 640, 3), np.array([[640, 640]])], training=False)
...
for i in range(len(images)):
    ...
    image_util.draw_bounding_boxes_and_masks_from_array("img-train-{}.png".format(i), image, boxes, masks, texts)
...
for i in range(len(images)):
    ...
    image = images[i][:img_sizes[i][0], :img_sizes[i][1]]
    image_util.draw_bounding_boxes_and_masks_from_array("img-valid-{}".format(i), image, boxes, masks, texts)
